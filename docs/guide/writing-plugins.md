...
> **适合谁**：想写业务命令 / 功能的 Python 开发者。建议先按 [安装](./installation.md) 启动成功、看到「框架启动完成」日志，再回来。
>
> 这篇会用一个**完整的「每日签到」插件**从零带你写：每个知识点先给能跑的代码，再解释为什么这样写、不写会出什么问题。读完你就能独立写任意插件。
...
2. 到 Web 面板「插件」页点 **重载**（或重启框架）。
3. 发命令测试。

**改了代码后**：再点一次「重载」即可，不用重启整个框架。
