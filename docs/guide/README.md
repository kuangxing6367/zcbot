...
>
> **OneBot 11（QQ）只是默认接入端，不是身份。** 换一个 `ProtocolAdapter`，即可接入
...
> 目标：把宿主跑起来、连上 QQ（或不连）、会装插件、会改配置、会用后台。

1. [安装](./installation.md) —— 环境、依赖、目录结构
2. [开始使用](./getting-started.md) —— 启动、连 QQ、第一次对话、Web 后台
...
### 角色 C：我要接非 QQ 的事件源 / 写自己的接入端（高级开发者）
...
- **协议无关**：业务插件优先用 `ctx.send_msg / ctx.api` 这类中立接口，不直接依赖 QQ 特有字段，换接入端时零改动。
