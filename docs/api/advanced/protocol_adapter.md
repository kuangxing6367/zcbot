...
想接一个新事件源时打开这页：Telegram、Discord、MQTT、HTTP Webhook……写一个 `ProtocolAdapter` 接进去，业务插件完全感知不到接入端换了。框架核心不绑定任何具体 IM 协议（`framework/` 内没有任何 OneBot 实现）。
