...
ZCBOT 的 API 分为两大部分：**基础参考**（写插件每天都会用到的对象）与**进阶扩展**（扩展平台内核行为、往内核运行环节插行为的契约）。
...
| 文档 | 内容 |
| ---- | ---- |
| [扩展点（Hook 系统）](./advanced/hooks.md) | 内核最核心的契约：在启动/关闭、Web 请求、事件分发、命令执行、协议动作、出站文本等几乎每个环节插入自己的逻辑 |
