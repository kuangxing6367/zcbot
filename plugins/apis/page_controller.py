...
from plugins.apis.api_aggregator import APICoreApp
from plugins.apis.api_aggregator.model import ItemsBatch, NamesBatch, TargetsBatch, UpdateItemsBatch
...
class APIPageController:
    ...
    def register_routes(self, flask_app) -> None:
        """在 Flask 应用中注册所有路由"""
        routes = [
            ("/api/plugins/apis/page/pool", self.get_pool, ["GET"], "Get API pool"),
            ("/api/plugins/apis/page/pool/files", self.get_pool_files, ["GET"], "Get pool files"),
            (
                "/api/plugins/apis/page/pool/files/delete",
                self.delete_pool_files,
                ["POST"],
                "Delete pool files",
            ),
            (
                "/api/plugins/apis/page/pool/export/<pool_type>",
            ...
            (
                "/api/plugins/apis/page/pool/import/<pool_type>",
                self.import_pool_file,
                ["POST"],
                "Import pool file",
            ),
            (
                "/api/plugins/apis/page/pool/import/<pool_type>/path",
                self.import_pool_from_default_path,
                ["POST"],
                "Import pool from default path",
            ),
            (
                "/api/plugins/apis/page/editor/site-form",
                self.site_form,
                ["GET"],
                "Get site form template",
            ),
            (
                "/api/plugins/apis/page/editor/api-form",
                self.api_form,
                ["GET"],
                "Get api form template",
            ),
            ("/api/plugins/apis/page/site/batch", self.create_sites_batch, ["POST"], "Create sites"),
            ("/api/plugins/apis/page/site/batch", self.update_sites_batch, ["PUT"], "Update sites"),
            ("/api/plugins/apis/page/site/batch", self.delete_sites_batch, ["DELETE"], "Delete sites"),
            ("/api/plugins/apis/page/api/batch", self.create_apis_batch, ["POST"], "Create apis"),
            ("/api/plugins/apis/page/api/batch", self.update_apis_batch, ["PUT"], "Update apis"),
            ("/api/plugins/apis/page/api/batch", self.delete_apis_batch, ["DELETE"], "Delete apis"),
            ("/api/plugins/apis/page/test/stream", self.test_api_stream, ["GET"], "Test API stream"),
            (
                "/api/plugins/apis/page/test/preview/batch",
                self.test_api_preview_batch,
                ["POST"],
                "Preview test API batch",
            ),
            ("/api/plugins/apis/page/local-file", self.local_file, ["GET"], "Get local file"),
            (
                "/api/plugins/apis/page/local-file/content",
                self.local_file_content,
                ["GET"],
                "Get local file content",
            ),
            ("/api/plugins/apis/page/local-data", self.get_local_data, ["GET"], "Get local data"),
            (
                "/api/plugins/apis/page/local-data/items/batch",
                self.get_local_data_items_batch,
                ["POST"],
                "Get local data items batch",
            ),
            (
                "/api/plugins/apis/page/local-data/batch",
                self.delete_local_data_batch,
                ["POST", "DELETE"],
                "Delete local data batch",
            ),
            (
                "/api/plugins/apis/page/local-data-item/batch",
        ...
        for route, handler, methods, desc in routes:
            # 为每个路由创建一个视图函数包装器
            view_func = self._make_view_func(handler)
            view_func.__name__ = f"apis_{handler.__name__}"
            view_func.__doc__ = desc
            flask_app.route(route, methods=methods)(view_func)

    def _make_view_func(self, handler):
        """创建视图函数包装器，将 handler 转为 Flask 视图"""
        def view_fn(**kwargs):
            return handler(**kwargs)
        view_fn.__name__ = f"apis_{handler.__name__}"
        return view_fn
    ...
    async def _send_text_file(self, path: Path, content_type: str):
        ...
        return Response(text, mimetype=content_type)
    ...
    async def delete_pool_files(self):
        try:
            ...
            self.core.cfg._ctx and self.core.cfg._ctx.log(f"[api_aggregator] delete pool files failed: {exc}")
    ...
    async def export_pool_file(self, pool_type: str):
        try:
            ...
            self.core.cfg._ctx and self.core.cfg._ctx.log(f"[api_aggregator] export pool failed: {exc}")
    ...
    async def import_pool_file(self, pool_type: str):
        try:
            ...
            self.core.cfg._ctx and self.core.cfg._ctx.log(f"[api_aggregator] import pool failed: {exc}")
    ...
    async def import_pool_from_default_path(self, pool_type: str):
        try:
            ...
            self.core.cfg._ctx and self.core.cfg._ctx.log(f"[api_aggregator] import by path failed: {exc}")
            return self._error(f"import failed: {exc}", status=500)
    ...
    async def test_api_stream(self):
        ...
        async def generate():
            try:
                ...
                self.core.cfg._ctx and self.core.cfg._ctx.log(f"[api_aggregator] test stream failed: {exc}")
    ...
    async def local_file(self):
        try:
            target = self.local.resolve_local_file(request.args.get("path", ""))
