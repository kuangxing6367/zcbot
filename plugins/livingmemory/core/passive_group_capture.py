...
import logging
logger = logging.getLogger(__name__), sp
...
async def is_plugin_enabled_for_session(session_id: str) -> bool:
    ...
    try:
        session_plugin_config = {}  # removed AstrBot sp dependency
    except Exception as exc:
...
class PassiveGroupCaptureFilter():
    ...
    @staticmethod
    def _passes_global_whitelist(event: Any, cfg) -> bool:
        platform_settings = (
    ...
    def filter(self, event: Any, cfg) -> bool:
