...
from plugins.livingmemory.storage.faiss_vec_db import FaissVecDB
...
class VectorRetriever:
    ...
    async def _get_uuid_from_id(self, doc_id: int) -> str | None:
        ...
        try:
            # 直接查询本地 sqlite3
            cursor = self.faiss_db._conn.execute(
                "SELECT doc_id FROM documents WHERE id = ?", (doc_id,)
            )
            row = cursor.fetchone()
            if not row:
                return None
