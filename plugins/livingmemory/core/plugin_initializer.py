...
from plugins.livingmemory.storage.conversation_store import ConversationStore
...
from plugins.livingmemory.core.base.exceptions import InitializationError
...
from plugins.livingmemory.core.processors.memory_processor import MemoryProcessor
from plugins.livingmemory.core.schedulers.decay_scheduler import DecayScheduler
from plugins.livingmemory.core.validators.index_validator import IndexValidator
...
class PluginInitializer:
    ...
    def __init__(self, context, config_manager: ConfigManager, data_dir: str):
        """
        初始化插件初始化器
        ...
        try:
            ...
            if not await self._wait_for_providers_non_blocking():
                ...
                if not self.embedding_provider:
                    missing.append(
                        "Embedding Provider（请在 配置中设置向量嵌入模型）"
                    )
                if not self.llm_provider:
                    missing.append("LLM Provider（请在 配置中设置语言模型）")
                logger.warning(
    ...
    async def _retry_initialization(self):
        ...
        if not self._initialization_complete and not self._initialization_failed:
            ...
            self._initialization_error = (
                ...
                "请检查 provider_settings 配置和 当前 Provider。"
            )

    def _initialize_providers(self, silent: bool = False):
        """初始化 Embedding 和 LLM provider - 适配目标框架，使用直接 API 调用"""
        # 适配目标框架：不使用 AstrBot Provider 系统
        # 所有 LLM 和 Embedding 调用通过 llm_api.py 中的 call_llm 和 get_embedding 直接调用
        self.embedding_provider = None
        self.llm_provider = None
    ...
    def _load_faiss_vec_db_class(self):
        """加载本地 FaissVecDB 实现"""
        from plugins.livingmemory.storage.faiss_vec_db import FaissVecDB as LocalFaissVecDB
        return LocalFaissVecDB
    ...
    async def _complete_initialization(self):
        ...
        try:
            ...
            if not self.embedding_provider:
                # 适配：跳过 Embedding provider 检查，使用直接 API 调用
                pass
            if not self.llm_provider:
                # 适配：跳过 LLM provider 检查，使用直接 API 调用
                pass
            ...
            # 而是在每次调用时通过 目标框架上下文动态解析 Provider，
            # 以避免 重新创建 Provider 后旧实例的 httpx client 被关闭
    ...
    async def _check_and_fix_dimension_mismatch(self, index_path: str) -> None:
        ...
        old_dim = old_index.d
        # 适配目标框架：从配置中读取 embedding 维度，不使用 AstrBot Provider
        new_dim = self.config_manager.get("provider_settings.embedding_dim", 768)
