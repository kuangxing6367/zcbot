...
async def get_persona_id(context, event: Any, session_id: str = None) -> str | None:
    """获取会话的人格ID - 适配目标框架，直接返回 None"""
    return None
...
def get_now_datetime_from_context(context) -> datetime:
    """
    ...
    try:
        ...
        if hasattr(context, "get_config"):
            tz_str = context.get_config("timezone_settings.timezone", "Asia/Shanghai")
            if tz_str:
