...
from plugins.livingmemory.core.i18n_backend import t, t_list
...
class CommandHandler:
    ...
    async def handle_search(
        self, event: Any, query: str, k: int = 5
    ) -> None:
        """处理 /lmem search 命令"""
        if not self.memory_engine:
            yield event.plain_result(
    ...
    async def handle_summarize(
        ...
        try:
            ...
            # 获取 persona_id
            from plugins.livingmemory.core.utils import get_persona_id
    ...
    async def handle_cleanup(
        self, event: Any, dry_run: bool = False
    ) -> None:
