...
from plugins.livingmemory.core.utils import (
    OperationContext,
...
if TYPE_CHECKING:
    ...
    from plugins.livingmemory.core.managers.conversation_manager import ConversationManager
    from plugins.livingmemory.core.managers.memory_engine import MemoryEngine
    from plugins.livingmemory.core.utils.injection_adapter import InjectionAdapter
    from plugins.livingmemory.core.event_handler_modules.message_utils import MessageUtils
...
class MemoryRecall:
    ...
    def _get_session_id(self, event: Any) -> str:
        """从事件中获取会话ID - 适配目标框架"""
        # 尝试从事件中获取 unified_msg_origin (AstrBot兼容)
        session_id = getattr(event, "unified_msg_origin", None)
        if session_id:
            return session_id
        # 目标框架：使用 user_id + group_id 构建会话ID
        user_id = getattr(event, "user_id", None) or "unknown"
    ...
    def _is_group_event(self, event: Any) -> bool:
        """判断事件是否为群聊事件 - 适配目标框架"""
        # 尝试 AstrBot 兼容方式
        msg_type = getattr(event, "get_message_type", None)
        if msg_type:
            try:
                return msg_type() == "group_message"
            except Exception:
                pass
        # 目标框架方式
    ...
    async def handle_memory_recall(
        ...
        try:
            ...
            # 当 req 为 None 时，使用 event 作为 LLM 请求对象（适配 llm_request 事件）
            llm_req = req if req is not None else event
            ...
            async with OperationContext("记忆召回", session_id):
                prompt_text = getattr(llm_req, "prompt", "")
                extra_parts = getattr(llm_req, "extra_user_content_parts", [])
                ...
                normalized = self._normalize_text_only_context_parts(llm_req, session_id)
                if normalized > 0:
                ...
                if self.config_manager.get("recall_engine.auto_remove_injected", True):
                    ...
                    removed += self._remove_fake_tool_call_from_context(llm_req, session_id)
                    if removed > 0:
                        logger.info(
                ...
                if not is_group and actual_query:
                    ...
                    if not message_to_store:
                        message_to_store = (
                            await self.message_utils.extract_message_content(event, llm_req)
                        )
                    if not message_to_store:
                ...
                # 获取 persona_id（适配目标框架: get_persona_id 返回 None）
                ...
                if recalled_memories:
                    ...
                    # 根据配置选择注入方式（适配目标框架：不使用 Provider 兼容降级）
                    ...
                    injection_method, fallback_reason = (
                        self.injection_adapter.resolve(None, configured_method)
                    )
                    if fallback_reason:
                        logger.warning(
                    ...
                    if injection_method == "user_message_before":
                        ...
                        if fake_messages:
                            if not hasattr(llm_req, "contexts"):
                                llm_req.contexts = []
                            llm_req.contexts.extend(fake_messages)
                            logger.info(
                        ...
                        # 适配目标框架：直接追加字符串到 extra_user_content_parts
                        if not hasattr(llm_req, "extra_user_content_parts"):
                            llm_req.extra_user_content_parts = []
                        llm_req.extra_user_content_parts.append(memory_str)
                        logger.info(
    ...
    def _remove_injected_memories_from_context(
        self, req: Any, session_id: str
    ) -> int:
        """从请求上下文中移除临时注入的记忆片段"""
        import re
        from plugins.livingmemory.core.base.constants import MEMORY_INJECTION_FOOTER, MEMORY_INJECTION_HEADER
    ...
    def _remove_fake_tool_call_from_context(
        ...
        """
        from plugins.livingmemory.core.base.constants import FAKE_TOOL_CALL_ID_PREFIX
