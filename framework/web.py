...
def create_web_app(framework) -> Flask:
    ...
    def login():
        ...
        db.execute(
            "UPDATE admin_users SET last_login_at = NOW(), last_login_ip = %s WHERE id = %s",
            (get_client_ip(), row['id'])
        )
        ...
        return jsonify({
            'code': 0,
            'msg': '登录成功',
            'data': {'username': username, 'role': row['role']}
        })
    ...
    def me():
        """获取当前登录信息"""
        return jsonify({'code': 0, 'data': session.get('admin')})
    ...
    def clear_runtime_logs():
        ...
        try:
            log_broker.clear()
            audit_log(session['admin']['id'], session['admin']['username'],
    ...
    @require_auth
    def logs_sse():
        """SSE 实时日志推送端点（参考 AstrBot /api/live-log）"""
        def generate():
